...
class BlueBlob(Blob):
    ...
    def __add__(self, other_blob):
        logging.info('Blob add operation of {} + {}'.format(repr(self), repr(other_blob)))
    ...
    for blue_id, blue_blob, in blues.copy().items():
        for other_blobs in blues, reds, greens:
            for other_blob_id, other_blob in other_blobs.copy().items():
                logging.debug(
                    'Checking if blobs are touching {} + {}'.format(repr(blue_blob), repr(other_blob)))
